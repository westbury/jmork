...
public class AliasesTest extends TestCase {
	...
	public void testAliasesStringDictDict1() {
		...
		Aliases aliases = new Aliases("(foo^80)", new Dicts(Collections
				.singletonList(dict)));
	...
	public void testAliasesStringDictDict3() {
		...
		Aliases aliases = new Aliases("(foo^81)", new Dicts(Collections
				.singletonList(dict)));
		assertEquals("bar", aliases.getValue("foo"));
	}
	...
	public void testAliasesStringDictDict4() {
		...
		Aliases aliases = new Aliases("(^80=bar)", new Dicts(Arrays.asList(new Dict[] {
				dict1, dict2 })));
		assertEquals("bar", aliases.getValue("foo"));
	}
